...
var HistoryProvider = /** @class */ (function () {
    ...
    HistoryProvider.prototype.getBars = function (symbolInfo, resolution, rangeStartDate, rangeEndDate) {
        var _this = this;
        var requestParams = {
            symbol: symbolInfo.ticker || 'X',
            toSymbol: symbolInfo.toSymbol || 'BTC',
            period: symbolInfo.period || '1min',
            resolution: resolution,
            from: rangeStartDate,
            to: rangeEndDate,
        };
        return new Promise(function (resolve, reject) {
            _this._requester.sendRequest(_this._datafeedUrl, '650066', requestParams)
                ...
                if (response.length <= 0) {
                    meta.noData = true;
                    meta.nextTime = Date.parse(new Date());
                }
                else {
...
//      'toSymbol': 'BTC',
//      'period': '1min'};
		var symbolInfo = {"name":"AAPL","exchange-traded":"NasdaqNM","exchange-listed":"NasdaqNM","timezone":"America/New_York","minmov":1,"minmov2":0,"pointvalue":1,"session":"0930-1630","has_intraday":false,"has_no_volume":false,"description":"Apple Inc.","type":"stock","supported_resolutions":["D","2D","3D","W","3W","M","6M"],"pricescale":100,"ticker":"AAPL"};
...
jQuery.fn.extend({
	...
	on: function( types, selector, data, fn, /*INTERNAL*/ one ) {
		...
		if ( typeof types === "object" ) {
			...
			for ( type in types ) {
				this$1.on( type, selector, data, types[ type ], one );
			}
			return this;
		}
	...
	stop: function( type, clearQueue, gotoEnd ) {
		...
		return this.each(function() {
			...
			for ( index = timers.length; index--; ) {
				if ( timers[ index ].elem === this$1 && (type == null || timers[ index ].queue === type) ) {
...
jQuery.fn.extend({
	...
	hasClass: function( selector ) {
		...
		var className = " " + selector + " ",
			...
			l = this.length;
		for ( ; i < l; i++ ) {
			if ( this$1[i].nodeType === 1 && (" " + this$1[i].className + " ").replace(rclass, " ").indexOf( className ) >= 0 ) {
				return true;
			}
		}
